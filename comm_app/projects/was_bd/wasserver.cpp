...
void ClientAccessServer::processMsgGb808(socket_t *sock ,const char *data, int len, const string &str_car_id)
{
	...
	if (!user._user_id.empty()) {
		...
		if (user._fd != sock) {
			...
			OUT_ERROR(ip, port, str_car_id.c_str(),
					"fd %d not equal socket old fd %d", sock->_fd,
					user._fd->_fd);
		...
		OUT_ERROR(ip, port, str_car_id.c_str(), "term no auth, close it!!!");
		CloseSocket(sock);
		return;
	}
	...
	if ( msg_id == 0x0101 ||  msg_id == 0x0003 )
	{
		...
		// 需要处理通用应答
		bresp  = true ;
	} else if(msg_id == 0x0107) { //查询终端属性应答
		if( msg_len < 48) { //最小长度
			OUT_ERROR( ip, port, str_car_id.c_str(), "Term attribute data length %d error" , msg_len );
			return;
		}

		string caits = "CAITS 0_0 " + caits_car_id + " 0 D_GETP {TYPE:1,RET:0";
		caits += _gb_proto_handler->getTermAttribute(data, len);
		caits += "} \r\n";
		...
		result = 0x00;
		bresp = true;
	} else if(msg_id == 0x0108) { //终端升级结果通知
		if( msg_len != 2) {
			OUT_ERROR( ip, port, str_car_id.c_str(), "TermUpdateReply data length %d error" , msg_len );
			return;
		}

		TermUpdateReply *updateReply = (TermUpdateReply*)(data + sizeof(GBheader));

		string caits = "CAITS 0_0 " + caits_car_id + " 0 U_REPT {TYPE:61";
		caits += ",660:" + uitodecstr(updateReply->type);
		caits += ",661:" + uitodecstr(updateReply->result);
		caits += "} \r\n";
		...
		string key = str_car_id + "_" + uitodecstr( respid );
		...
		// 需要处理通用应答
		bresp  = true ;
	} else if(msg_id == 0x0704) {//定位数据批量上传
		if(msg_len < sizeof(BatchLocationHead)) {
			OUT_ERROR( ip, port, str_car_id.c_str(), "0x0704 mesage length %d error", msg_len );
			return;
		}

		uint16_t i;
		size_t fieldPos = sizeof(GBheader);
		BatchLocationHead *batHead = (BatchLocationHead*)(data + fieldPos);
		uint16_t batSize = ntohs(batHead->size);
		BatchLocationItem *itemPtr;
		uint16_t itemLen;

		// 0：正常位置批量汇报，1：盲区补报
		string locType = batHead->type == 0 ? "0" : "7";

		fieldPos += sizeof(BatchLocationHead);
		for(i = 0; i < batSize && fieldPos + sizeof(BatchLocationItem) < (size_t)len; ++i) {
			itemPtr = (BatchLocationItem*)(data + fieldPos);
			itemLen = ntohs(itemPtr->len);

			fieldPos += sizeof(BatchLocationItem);
			if(fieldPos + itemLen > (size_t)len) { //验证长度指示是否正确
				break;
			}

			if(itemLen < sizeof(GpsInfo)) { //是否满足最小长度
				break;
			}

			char *appendPtr = (char*)itemPtr->dat + sizeof(GpsInfo);
			int   appendLen = itemLen - sizeof(GpsInfo);

			string caits = "CAITS 0_0 " + caits_car_id + " 0 U_REPT {TYPE:" + locType + ",";
			caits += _gb_proto_handler->ConvertGpsInfo((GpsInfo*)itemPtr->dat, appendPtr, appendLen) ;
			caits += "} \r\n";
		...
		result = 0x00 ;
		bresp  = true ;
	} else if(msg_id == 0x0705) {//CAN 总线数据上传
		if(msg_len < sizeof(CanData)) {
			OUT_ERROR( ip, port, str_car_id.c_str(), "0x0705 mesage length %d error", msg_len ) ;
			return;
		}

		CanData *canData = (CanData*)(data + sizeof(GBheader));
		CanItem *canItem;

		uint16_t i;
		uint16_t itemSize = ntohs(canData->size);
		uint32_t itemPos  = sizeof(GBheader) + sizeof(CanData);

		char canKey[9];
		char canVal[17];
		vector<char> datBuf;

		datBuf.reserve(itemPos * 32);

		for(i = 0; i < itemSize && itemPos + sizeof(CanItem) < (size_t)len; ++i) {
			canItem = canData->item + i;
			_gb_proto_handler->bin2hex(canItem->key, 4, canKey);
			_gb_proto_handler->bin2hex(canItem->val, 8, canVal);

			if( ! datBuf.empty()) {
				datBuf.push_back('|');
			}

			datBuf.insert(datBuf.end(), canKey, canKey + 8);
			datBuf.push_back(':');
			datBuf.insert(datBuf.end(), canVal, canVal + 16);
		}

		string caits = "CAITS 0_0 " + caits_car_id + " 0 U_REPT {TYPE:60,650:";
		caits += BCDtostr((char*)canData->bcd, 5);
		if( ! datBuf.empty()) {
			CBase64 base64;
			base64.Encode( &datBuf[0], datBuf.size() );

			caits += ",651:" + string(base64.GetBuffer());
		}
		caits += "} \r\n";
	...
	} else if ( msg_id == 0x0805 ) {
		// 如果失败，没有多媒体id个数字段
		if( len < (int)(sizeof(TakePhotoReply) - sizeof(short))) {
			OUT_WARNING( ip, port, str_car_id.c_str(), "TakePhotoApply mesage length %d error", len ) ;
			return;
		}

		TakePhotoReply *tpReply = (TakePhotoReply*)data;
		respid = ntohs(tpReply->ackid);

		string key = str_car_id + "_" + uitodecstr(respid);
		CacheData cache_req = _cache_data_pool.checkData(key);
		if(cache_req.str_send_msg == "EXIT" ) {
			string caits = "CAITR " + cache_req.seq + " " + caits_car_id + " 0 " + cache_req.command;
			// 和目前平台兼容，只返回状态
			caits += " {RET:" + uitodecstr(tpReply->result) + "} \r\n";

			_pEnv->GetMsgClient()->HandleUpData( caits.c_str(),caits.length() ) ;
		} else {
			//OUT_WARNING(ip,port,str_car_id.c_str(),"msg type: %x, msg id: 0x8801, cant's find seq:%x in cache_data_pool", msg_id, respid );
			sendOtherCmd(caits_car_id, msg_id, data + sizeof(GBheader), msg_len);
		}
		...
		for(i = 0; i < info_count; ++i) {
	    	...
	    	if(info_id == 0xb) {
	    		infos[info_id - 1] = Utils::filter(string(1, info_buf[0] + '0'), filter);
	    	} else if(info_id >= 1 && info_id <= 0xe) {
		...
		result = 0x00;
		bresp  = true;
	} else if (msg_id == 0x0f16) { // 远程锁车、解锁、查询应答
		if ( len < (int)sizeof(RemoteCtrlCarReply)) {
			OUT_ERROR( ip, port, str_car_id.c_str(), "remote control car reply length %d error", len ) ;
			return ;
		}

		RemoteCtrlCarReply *reply = (RemoteCtrlCarReply*)data;
		unsigned short ackseq = ntohs(reply->ackseq);
		unsigned short method = ntohs(reply->method);

		respid = ackseq;

		string rspArgs;
		if(method == 1 && msg_len == 4 + 1 ) { // 锁车失败应答或解锁应答，没有6字节的解锁码
			RemoteCtrlCarLockReply *lockReply = (RemoteCtrlCarLockReply*)(data + sizeof(RemoteCtrlCarReply));
			rspArgs = "RET:" + string(1, lockReply->result + '0');
		} else if(method == 1 && msg_len == 4 + 1 + 6 ) { // 锁车成功应答，返回6字节的锁车码
			RemoteCtrlCarLockReply *lockReply = (RemoteCtrlCarLockReply*)(data + sizeof(RemoteCtrlCarReply));
			rspArgs = "RET:" + string(1, lockReply->result + '0');
			if(lockReply->result == 0) {
				rspArgs += ",VALUE:" + string(lockReply->code, 6);
			}
		} else if(method == 2 && msg_len == 4 + sizeof(RemoteCtrlCarParam)) { // 查询当前锁车设置
			RemoteCtrlCarParam *param = (RemoteCtrlCarParam*)(data + sizeof(RemoteCtrlCarReply));
			rspArgs = "RET:0,VALUE:";
			rspArgs += string(1, '0' + param->action);
			rspArgs += "|" + string(1, '0' + param->channel);

			if(param->maxrpm == 0xffff) {
				rspArgs += "|-1"; //无效值
			} else {
				rspArgs += "|" + uitodecstr(ntohs(param->maxrpm));
			}

			if(memcmp(param->locktime, "\xff\xff\xff\xff\xff\xff", 6) == 0) {
				rspArgs += "|-1"; //无效值
			} else {
				rspArgs += "|20" + BCDtostr((char*)param->locktime, 6);
			}

			if(memcmp(param->locktime, "\xff\xff\xff\xff\xff\xff", 6) == 0) {
				rspArgs += "|-1"; //无效值
			} else {
				rspArgs += "|20" + BCDtostr((char*)param->warntime, 6);
			}

			rspArgs += "|" + uitodecstr(param->status);
		} else {
			OUT_WARNING(ip, port, str_car_id.c_str(), "remote control car reply error, method: %d", method);
			return;
		}

		string key = str_car_id + "_" + uitodecstr(ackseq);
		CacheData cache_req = _cache_data_pool.checkData(key);
		if(cache_req.str_send_msg == "EXIT" ) {
			string caits = "CAITR "+ cache_req.seq + " " + caits_car_id + " 0 " + cache_req.command;
			caits += " {" + rspArgs + "} \r\n";
			...
			//OUT_WARNING(ip,port,str_car_id.c_str(), "msgid: %04x, cant's find seq: %x in cache_data_pool", msg_id, ackseq );
			sendOtherCmd(caits_car_id, msg_id, data + sizeof(GBheader), msg_len);
		}
	} else if (msg_id == 0x0f17) {
		if(len != sizeof(RemoteCtrlCarReport)) {
			OUT_ERROR( ip, port, str_car_id.c_str(), "RemoteCtrlCarReport length %d error", len);
			return;
		}

		RemoteCtrlCarReport *report = (RemoteCtrlCarReport*)data;

		string status(1, '0' + report->status);
		string caits = "CAITS 0_0 " + caits_car_id + " 0 U_REPT {TYPE:62,571:" + status;
		caits += "," + _gb_proto_handler->ConvertGpsInfo(&report->gbsinfo, NULL, 0) + "} \r\n";
		_pEnv->GetMsgClient()->HandleUpData( caits.c_str(),caits.length() );
		...
		if ( msg_len < sizeof(SpeedAdjustReport)) {
		    OUT_ERROR( ip, port, str_car_id.c_str(), "speed adjust report message length %d error", msg_len) ;
		    return ;
		}
...
bool ClientAccessServer::SendDataToUser( const string &user_id,const char *data,int data_len)
{
	...
	//OUT_SEND3( user._ip.c_str() , user._port , user._user_id.c_str() , "fd %d send response len %d", user._fd->_fd , data_len ) ;
	//OUT_HEX( user._ip.c_str() , user._port , user._user_id.c_str() , data , data_len );
