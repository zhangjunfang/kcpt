...
void WasClient::on_data_arrived(socket_t *sock, const void *data, int len)
{
	...
	case 0x8100:
		...
		if(reg_rsp->res == 0) { //注册成功往redis保存鉴权码
			...
			_pEnv->GetRedisCache()->HSet("FORWARD.AUTH", user._user_id.c_str(), authCode.c_str());
		}
		break;
	case 0x8300:
...
int WasClient::build_login_msg( User &user, char *buf,int buf_len )
{
	...
	if(user._user_pwd.empty()) {
		...
		// 发送鉴权消息
		string authCode = "";
		if( ! _pEnv->GetRedisCache()->HGet("FORWARD.AUTH", user._user_id.c_str(), authCode)) {
			authCode = "1234567890";
			OUT_WARNING(user._ip.c_str(), user._port, user._user_id.c_str(), "not found auth code");
		}
		_protocol.buildGb808Login(user._user_name, authCode, msg808Arr);
	}
...
void WasClient::registerTerm()
{
	...
	for(itSs = userids.begin(); itSs != userids.end(); ++itSs) {
		...
		valueStr.clear();
		_pEnv->GetRedisCache()->HGet("FORWARD.AUTH", userid.c_str(), valueStr);
