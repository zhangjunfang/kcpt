...
void UserMgr::run(void *param)
{
	...
	while(true) {
		...
		while(getline(ifs, line)) {
			...
			const string domain = "FORWARD.LIST." + fields[1];

			keys.clear();
			if( ! _pEnv->GetRedisCache()->HKeys(domain.c_str(), keys) || keys.empty()) {
				continue;
			}
			...
			for(itVs = keys.begin(); itVs != keys.end(); ++itVs) {
				val = "";
				if( ! _pEnv->GetRedisCache()->HGet(domain.c_str(), itVs->c_str(), val)) {
					continue;
				}
