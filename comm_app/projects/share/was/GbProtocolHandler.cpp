...
bool GbProtocolHandler::buildParamSet( DataBuffer *pbuf , map<string,string> &p_kv_map, unsigned char &pnum )
{
	...
	for (MapIter p = p_kv_map.begin(); p != p_kv_map.end(); ++p) {
		...
		case 18: //设置GPS数据回传间隔
			...
			break;
		case 31: //补页新增电子围栏半径
			setword(pbuf, 0x0031, atoi(v.c_str()));
			++pnum;
			break;
		case 41: //设置车牌号
