...
void WasServer::TimeWork()
{
	while (Check()) {
		...
		for(int i = 0; i < (int)users.size(); i++) {
			...
			OUT_INFO(user._ip.c_str(), user._port, "TOUT", "%s timeout and close it", user._user_id.c_str());
...
// 从BCD码中取得手机号
static bool bcd2sim(const uint8_t *bcd, string &sim)
{
...
void WasServer::on_data_arrived(socket_t *sock, const void *data, int len)
{
	...
	if (msgid == 0x0100) { //终端注册
		...
		if (result == 0) {
			...
			if (_online_user.AddUser(simid, user)) {
				...
				_online_user.SetUser(simid, user);
			}
		}
	...
	User user = _online_user.GetUserByUserId(simid);
	if(user._user_id.empty()) {
		OUT_WARNING(ip, port, "STAT", "%s not login, close it!!!", simid.c_str());
		return;
	}
	user._last_active_time = time(NULL);
	_online_user.SetUser(simid, user);

	_pEnv->GetWasClient()->HandleData(simid, (uint8_t*)data, len);
}
...
bool WasServer::HandleData(const string &sim, const uint8_t *ptr, size_t len)
{
	...
	return SendData(user._fd, (char*)ptr, len);
}
